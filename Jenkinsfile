pipeline {
  ...
  environment {
    ...
    DOCKER_HUB_USER = 'bnivasreddy'
    DOCKER_CREDENTIAL_ID = 'Srinivas2'
  ...
  post {
    ...
    sucess {
      slackSend(color: "good", message: "${env.JOB_NAME} completed sucessfully, details at ${env.RUN_DISPLAY_URL}"}
                }
                failure
                {
